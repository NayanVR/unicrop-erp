...
export default function PartiesIndex() {
    ...
    const deleteParty = (p: Party) => {
        if (!confirm(`Delete party "${p.name}"? All their documents will also be removed.`)) return;
        router.delete(partyDestroy(p.id).url);
    };

    const openDocs = (p: Party) => {
        setSelectedParty(p);
        ...
        <ErpLayout>
            ...
            <div className="card">
                ...
                <table className="data-table">
                    ...
                    <tbody>
                        {filtered.length === 0 && (
                            <tr><td colSpan={8} className="empty-row">No parties found.</td></tr>
                        )}
                        {filtered.map((p) => (
                            <tr key={p.id}>
                    ...
                    <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={submit} className="modal-form">
                            ...
                            <div className="form-row">
                                <div className="form-group">
                                    <label>GST No.</label>
                                    <input value={data.gst_no} onChange={(e) => setData('gst_no', e.target.value)} placeholder="22AAAAA0000A1Z5" />
                                </div>
                                <div className="form-group">
                                    <label>PAN No.</label>
                                    <input value={data.pan_no} onChange={(e) => setData('pan_no', e.target.value)} placeholder="AAAAA0000A" />
                                </div>
                                <div className="form-group">
                                    <label>Phone</label>
                            ...
                            <div className="modal-actions">
                                ...
                                <button type="submit" className="btn-primary" disabled={processing}>
                                    {processing ? 'Saving…' : editing ? 'Update' : 'Add Party'}
                                </button>
                            </div>
                        </form>
                    </div>
